...
export const assignProject = async (req, res) => {
    try
    {
        const { project_name, project_description, employees } = req.body;
        const project = await ProjectAssignModel.create({
            project_name,
            project_description,
            employees
        })
        return res.status(201).json({ message: "Project assigned successfuly", data:project });
    }
    catch (error) 
    {
...
export const getAssignedProjects = async (req, res) => {
    try
    {
        ...
        res.status(200).json({ page,limit,totalPages
            ,totalProjects, projects: project
         });
    }
    catch (error)
    {
...
export const updateAssignedProject = async (req, res) => {
    try
    {
        ...
        return res.status(200).json({ message: "Project update successfuly", data: project });
    }
    catch (error)
    {
...
export const deleteAssignedProject = async (req, res) => {
    ...
    {
        const { id } = req.params;  
        const project = await ProjectAssignModel.findByIdAndDelete(id);
        if (!project)
        {
            return res.status(400).json({ message: "Project not found" });
        }
